...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specularStrength = saturate(dot(-viewDirection,reflectedLight));
	specularStrength = pow(specularStrength,gShininess);
	if(gUseTextureSpecularIntensity)
	{
		specularStrength *= gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	float3 binormal = cross(tangent,newNormal);
	...
	float4 sampledNormal = gTextureNormal.Sample(gTextureSampler,texCoord)*2-1;
...
float CalculateFresnelMask(float3 n){
	float3 invUpVector = float3(0.f,-1.f,0.f);
	float dotInvUpVecAndN = abs(dot(invUpVector,n));
	float mask = pow(1.f - saturate(dotInvUpVecAndN),gFresnelHardness);
	return mask;
}

float CalculateFresnel(float3 n, float3 v)
{
	float saturatedDotNV = saturate(abs(dot(n,v)));
	float fresnel = pow((1-saturatedDotNV),gFresnelPower)*gFresnelMultiplier;
	return fresnel;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	{
		float3 fresnel = CalculateFresnel(normal,viewDirection);
		float3 fresnelMask = CalculateFresnelMask(normal);
